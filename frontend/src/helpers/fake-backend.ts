...
const usersKey = 'jobit-users'
let users = JSON.parse(localStorage.getItem(usersKey) || '[]')

function fakeBackend() {
  let realFetch = window.fetch
  window.fetch = function (url: any, opts: any): Promise<Response> {
    return new Promise((resolve, reject) => {
      ...
      function updateUser() {
        ...
        if (
          ...
          users.find((x: { username: string }) => x.username === params.username)
        ) {
      ...
      function ok(body: any = {}) {
        resolve(
          new Response(JSON.stringify(body), { status: 200, statusText: 'OK', headers: headers() }),
        )
      }
